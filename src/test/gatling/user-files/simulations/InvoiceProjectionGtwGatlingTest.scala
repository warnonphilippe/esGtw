...
/**
 * Performance test for the InvoiceProjectionGtw entity.
 */
class InvoiceProjectionGtwGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the InvoiceProjectionGtw entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all invoiceProjectionGtws")
            .get("/api/invoice-projection-gtws")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new invoiceProjectionGtw")
            .post("/api/invoice-projection-gtws")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "code":"SAMPLE_TEXT"
                ...
                , "paymentAmount":"0"
                , "aggregateId":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_invoiceProjectionGtw_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created invoiceProjectionGtw")
                .get("${new_invoiceProjectionGtw_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created invoiceProjectionGtw")
            .delete("${new_invoiceProjectionGtw_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
